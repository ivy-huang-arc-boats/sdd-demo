...
<!--
  ...
  The GitHub issue is the source of truth once posted. Frontmatter above is
  machine-readable metadata: `approvers` are the EXPECTED reviewers (not proof of
  approval — the `approved` label set by the Action is); `tasks` is backfilled by
  slice-to-subissues after decomposition.
